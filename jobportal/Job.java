...
public abstract class Job {
    ...
    public boolean isEligible(JobSeeker jobSeeker) {
        if (!this.qualification.equalsIgnoreCase(jobSeeker.getEducation().getDegree())) {
            return false;
        }
    
        if (jobSeeker.getExperience().getDuration() < this.experienceYears) {
            return false;
        }
    
        List<String> seekerSkills = jobSeeker.getSkills().getSkillList();
        for (String requiredSkill : requiredSkillsList) {
            if (!seekerSkills.contains(requiredSkill)) {
                return false;
            }
        }
